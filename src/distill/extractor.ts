...
import type { DistillCandidate, ToolTraceEntry } from "../types.js";

export function loadFixtureSession(path: string): { sessionText: string; toolTrace: ToolTraceEntry[]; sessionId: string } {
  ...
  for (const p of candidates) {
    if (existsSync(p)) {
      try {
        toolTrace = JSON.parse(readFileSync(p, "utf8")) as ToolTraceEntry[];
        break;
      } catch { /* ignore */ }
    }
  }
...
export function extractCandidateFromFixture(path: string): DistillCandidate {
  ...
  if (sessionText.startsWith("---")) {
    ...
    if (end !== -1) body = sessionText.slice(end + 4).replace(/^\n/, "");
  }
  return {
