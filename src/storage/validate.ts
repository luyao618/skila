...
export function assertValidName(name: string): void {
  if (typeof name !== "string" || name.length === 0 || name.length > 64 || !NAME_REGEX.test(name)) {
    throw new StorageAdapterError(
      "E_INVALID_NAME",
      `invalid skill name: ${JSON.stringify(name)} — must match ${NAME_REGEX} and be 1–64 chars`
    );
  }
}
...
export function assertValidVersion(version: string): void {
  if (typeof version !== "string" || !SEMVER_REGEX.test(version)) {
    throw new StorageAdapterError(
      "E_INVALID_VERSION",
      `invalid version: ${JSON.stringify(version)} — must match strict semver (MAJOR.MINOR.PATCH)`
    );
  }
}
