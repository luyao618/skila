...
function fsyncFd(fd: number): void {
  try {
    ...
    throw new StorageAdapterError(
      "E_FSYNC",
      `fsync failed: ${(err as Error).message}`,
...
export function atomicWriteFileSync(target: string, data: string | Buffer): void {
  ...
  try {
    ...
    try { _ops.closeSync(fd); } catch { /* best-effort */ }
  }
    ...
    if (code === "EXDEV") {
      ...
      try {
        ...
        try {
          ...
          try { _ops.closeSync(fd2); } catch { /* best-effort */ }
        }
        _ops.renameSync(destTmp, target);
        try { unlinkSync(tmp); } catch { /* best-effort */ }
      } catch (innerErr) {
        try { if (existsSync(destTmp)) unlinkSync(destTmp); } catch { /* best-effort */ }
  ...
  try {
    ...
    try { _ops.closeSync(dirFd); } catch { /* best-effort */ }
  }
}
