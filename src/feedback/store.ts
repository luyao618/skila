...
import { mkdirSync, rmdirSync, existsSync, statSync, readFileSync, unlinkSync } from "node:fs";
...
async function acquireLock(timeoutMs: number, staleMs: number): Promise<void> {
  const lp = lockPath();
...
export function readFeedbackSync(): FeedbackStoreShape {
...
function recompute(entry: FeedbackEntry): void {
...
export async function incrementUsage(name: string, outcome: "success" | "failure" | "unknown" = "success"): Promise<FeedbackEntry> {
