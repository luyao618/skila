...
async function readBoundedBody(req: IncomingMessage): Promise<string | { tooLarge: true }> {
  let body = "";
  let received = 0;
  for await (const chunk of req) {
    received += (chunk as Buffer).length;
    if (received > MAX_BODY_BYTES) return { tooLarge: true };
    body += chunk;
  }
  return body;
}
...
export async function handleTranslateSkill(
  req: IncomingMessage,
  res: ServerResponse,
  skillName: string
): Promise<void> {
...
- Output ONLY the translated document, no explanations`;
  ...
  try {
    ...
    if (!body) { sseWrite(res, { type: "error", error: "empty upstream body" }); res.end(); return; }
...
export async function handleGetTranslateSettings(
...
export async function handlePutTranslateSettings(
  req: IncomingMessage,
  res: ServerResponse
): Promise<void> {
  const bodyOrLimit = await readBoundedBody(req);
  ...
  let payload: { targetLang?: string; customBaseUrl?: string; model?: string };
  try {
    payload = JSON.parse(bodyOrLimit);
  } catch {
    sendJson(res, 400, { error: "invalid JSON body" });
  ...
  if (typeof payload.targetLang === "string") updates.translateTargetLang = payload.targetLang;
  if (typeof payload.customBaseUrl === "string") updates.translateBaseUrl = payload.customBaseUrl || undefined;
  if (typeof payload.model === "string") updates.translateModel = payload.model || undefined;
