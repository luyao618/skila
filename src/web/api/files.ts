...
export async function handleGetFile(
  ...
  if (normalized.includes("..")) { sendJson(res, 400, { error: "path traversal not allowed" }); return; }

  const skillDir = dirname(skill.path);
  const abs = join(skillDir, normalized);
  if (!abs.startsWith(skillDir)) { sendJson(res, 400, { error: "path outside skill dir" }); return; }
