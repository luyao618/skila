...
export async function startServer(opts: ServeOptions = {}): Promise<{ port: number; close: () => Promise<void>; token: string }> {
  ...
  const port = await new Promise<number>((resolve, reject) => {
    ...
    const tryBind = () => {
      if (attempt > 65535) { reject(new Error("no free port available below 65536")); return; }
  ...
  const close = () => new Promise<void>((r, e) => {
...
function isAllowedOrigin(originHeader: string | undefined): boolean {
  if (!originHeader || originHeader === "null") return true; // Same-origin GETs from older clients omit Origin; only enforce when present.
  try {
...
async function route(req: IncomingMessage, res: ServerResponse, distDir: string, serverToken: string): Promise<void> {
  ...
  if (skillMatch) {
    ...
    if (method === "GET" && sub === "file") {
      ...
      await handleGetFile(req, res, name, filePath, serverToken); return;
    }
...
let cspCache: { distDir: string; value: string } | null = null;
let cspManifestWarned = false;

function buildCsp(distDir: string): string {
  if (cspCache && cspCache.distDir === distDir) return cspCache.value;
  ...
  try {
    const parsed = JSON.parse(readFileSync(manifestPath, "utf8")) as { scriptSrc?: unknown };
    ...
    if (!cspManifestWarned) {
      ...
      process.stderr.write(
        `skila: CSP manifest unusable (${manifestPath}): ${(err as Error).message}\n` +
...
function checkContentTypeJson(req: IncomingMessage, res: ServerResponse): boolean {
...
function checkContentTypeJsonOptional(req: IncomingMessage, res: ServerResponse): boolean {
