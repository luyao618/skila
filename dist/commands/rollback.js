...
export async function runRollback(name, toVersion) {
    const skill = findSkill(name);
    if (!skill)
        throw new Error(`rollback: skill not found: ${name}`);
    ...
    if (!histRaw) {
        ...
        if (!existsSync(histFile))
            throw new Error(`rollback: historical version not found: v${toVersion}`);
...
export function snapshotVersion(name, version, raw) {
    const home = ensureSkilaHome();
