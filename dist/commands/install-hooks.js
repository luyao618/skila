...
function resolveSkilaGlobalPath() {
...
export function runInstallHooks() {
    ...
    for (const [event, entries] of Object.entries(desiredHooks)) {
        ...
        for (const entry of entries) {
            const alreadyPresent = settings.hooks[event].some((h) => h.type === entry.type && h.command === entry.command);
            if (alreadyPresent) {
    ...
    if (skipped.length > 0)
        process.stdout.write(`  already present (skipped): ${skipped.join(", ")}\n`);
