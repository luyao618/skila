...
function checkSentinelConsistency() {
    ...
    if (!existsSync(p))
        return { name: "adapter-mode sentinel", ok: true, detail: "absent (will be set on first run)" };
...
function checkSkillsWritable() {
    ...
    try {
        ...
        return { name: "skills root writable", ok: false, detail: err.message };
    }
}
function checkPluginJson() {
    ...
    for (const p of candidates) {
        ...
        try {
            ...
            return { name: "plugin.json hook resolution", ok: false, detail: err.message };
        }
    }
...
async function checkPort(port = 7777) {
...
function checkFeedbackJson() {
    const home = ensureSkilaHome();
    const p = join(home, "feedback.json");
    if (!existsSync(p))
        return { name: "feedback.json", ok: true, detail: "absent" };
    try {
        const j = JSON.parse(readFileSync(p, "utf8"));
        ...
        return { name: "feedback.json", ok: false, detail: err.message };
    }
}
function checkLockLeaks() {
    const home = ensureSkilaHome();
    const stale = [];
    walk(home, (p, st) => {
        if (p.endsWith(".lock") && st && st.isDirectory()) {
            const m = Number(st.mtimeMs);
            if (Date.now() - m > 5 * 60 * 1000)
                stale.push(p);
        }
    });
    return { name: "lock leaks", ok: stale.length === 0, detail: stale.length === 0 ? "none" : stale.join(", ") };
}
function checkStaleStaging() {
    const dir = statusDir("staging");
    if (!existsSync(dir))
        return { name: "stale staging>30d", ok: true, detail: "no staging dir" };
    const stale = [];
    for (const entry of readdirSync(dir)) {
        const p = join(dir, entry);
        try {
            const st = statSync(p);
            if (Date.now() - st.mtimeMs > 30 * 24 * 3600 * 1000)
                stale.push(entry);
        }
        catch { }
    }
...
function checkJudgeCache() {
    const home = ensureSkilaHome();
    const cache = join(home, "judge-cache");
    if (!existsSync(cache))
        return { name: "judge cache>7d", ok: true, detail: "no cache" };
    const stale = [];
...
export async function runFixStorage(opts = {}) {
    ...
    if (!sentinel) {
        const mode = gitExists ? "git" : "flat";
        writeFileSync(sp, mode + "\n");
        return { reconciled: true, from: "none", to: mode, detail: "initialized sentinel from on-disk state" };
    }
    return { reconciled: false, from: sentinel, to: sentinel, detail: "already consistent" };
}
async function replayFlatIntoGit(flat, git, _home) {
    ...
    for (const name of readdirSync(root)) {
        let versions;
        try {
            versions = await flat.listVersions(name);
        }
        catch {
            continue;
        }
        ...
        for (const v of versions) {
            let content;
            try {
                content = await flat.getVersion(name, v.version);
            }
            catch {
                continue;
            }
...
export async function runSelftest() {
    ...
    try {
        ...
        if (!got.includes("selftest"))
            return { ok: false, detail: "read mismatch" };
