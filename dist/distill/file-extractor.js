 ...
 */
export function extractSupportingFiles(toolTrace) {
    const candidates = [];
    const seen = new Set();
    ...
    for (const entry of toolTrace) {
        ...
        if (existing) {
            ...
            if (normalized.length > existing.fullCmd.length)
                existing.fullCmd = normalized;
        }
        else {
    ...
    for (const entry of toolTrace) {
        ...
        if (!DOC_EXTS.has(ext) && !CONFIG_EXTS.has(ext))
            continue;
