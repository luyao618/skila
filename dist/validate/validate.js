...
export function validateSkillContent(raw, opts = {}) {
    ...
    try {
        ...
        throw new SkilaValidationError([e.message]);
    }
    const fm = parsed.frontmatter;
    if (!fm || typeof fm !== "object")
        errors.push("frontmatter not an object");
    if (!fm.name)
        errors.push("missing required: name");
    if (!fm.description)
        errors.push("missing required: description");
    if (fm.name && !isValidName(fm.name))
        errors.push(`invalid name: ${fm.name}`);
    if (fm.description && fm.description.length > 1024)
        errors.push(`description >1024 chars (${fm.description.length})`);
...
export function validateSkilaMetadata(meta) {
    ...
    else if (!/^\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(m.version))
